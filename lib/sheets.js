...
// 依分頁名稱取得其數字 sheetId。batchUpdate 的格式化類操作（例如設定背景色）
// 只能用數字 sheetId 指定範圍，不能直接用分頁名稱字串。
...
// 依「列範圍 + 背景色」清單，用 batchUpdate 的 repeatCell 一次套用多段格式化，
// 用於「不同員工以不同底色區分」：同一位員工的連續資料列套用同一種背景色。
// ranges 每個元素：{ startRowIndex, endRowIndex, startColumnIndex, endColumnIndex, color: {red,green,blue} }，
// 皆為 0-based、endIndex 不含（即 Sheets API 的慣例：[start, end)）。
async function applyRowBackgroundColors(spreadsheetId, sheetId, ranges) {
    if (!ranges || !ranges.length || sheetId === null || sheetId === undefined) return;
    ...
    await sheetsApiRequest('POST', `/v4/spreadsheets/${spreadsheetId}:batchUpdate`, { requests });
}
